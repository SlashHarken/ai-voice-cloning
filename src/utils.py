...
if BARK_ENABLED:
    ...
    class Bark_TTS():
        ...
        def inference(self, text, voice, text_temp=0.7, waveform_temp=0.7):
            ...
            audio_tokens = semantic_to_audio_tokens(semantic_tokens, history_prompt=voice, temp=waveform_temp,
                                                    silent=False, output_full=False)
...
def generate_bark(**kwargs):
    ...
    for line, cut_text in enumerate(texts):
        ...
        for j, g in enumerate(gen):
            ...
            audio_cache[name] = {
                'audio': wav,
                'settings': get_info(voice=override['voice'] if override and 'voice' in override else voice,
                                     settings=settings)
            }
...
def generate_valle(**kwargs):
    ...
    def get_settings(override=None):
        ...
        settings['references'] = fetch_voice(
            voice=selected_voice)  # [ fetch_voice(voice=selected_voice) for _ in range(3) ]
        return settings
...
def generate_tortoise(**kwargs):
    ...
    def fetch_voice(voice):
        ...
        if voice_samples and len(voice_samples) > 0:
            if conditioning_latents is None:
                conditioning_latents = compute_latents(voice=voice, voice_samples=voice_samples,
                                                       voice_latents_chunks=parameters['voice_latents_chunks'])
    ...
    def get_settings(override=None):
        ...
        if settings['conditioning_latents'] is not None and len(settings['conditioning_latents']) == 2 and settings[
            'cvvp_amount'] > 0:
            print(
    ...
    def get_info(voice, settings=None, latents=True):
        ...
        if latents and "latents" not in info:
            ...
            model_hash = settings["model_hash"][
                         :8] if settings is not None and "model_hash" in settings else tts.autoregressive_model_hash[:8]
        ...
        try:
            if args.use_hifigan:
                ...
                unused_args = ['diffusion_temperature', 'cond_free_k', 'sample_batch_size', 'diffusion_iterations',
                               'return_deterministic_state', 'diffusion_sampler', 'breathing_room', 'half_p',
                               'cond_free',
            ...
            raise RuntimeError(
                f'Possible latent mismatch: click the "(Re)Compute Voice Latents" button and then try again. Error: {e}')
        # print(type(gen))
        # print(gen)
        run_time = time.time() - start_time
        print(f"Generating line took {run_time} seconds")
...
def compute_latents(voice=None, voice_samples=None, voice_latents_chunks=0, original_ar=False,
...
class TrainingState():
        ...
        if self.it > 0:
            ...
            for k in keys['losses']:
                ...
                self.statistics['loss'].append(
                    {'epoch': epoch, 'it': self.it, 'value': self.info[k], 'type': f'{prefix}{k}'})
    ...
    def get_status(self):
        ...
        if False and len(self.losses) >= 2:
            ...
            if deriv != 0:  # dloss < 0:
                ...
                print(
                    f"Loss value: {loss_value} | Next milestone: {next_milestone} | Distance: {loss_value - next_milestone}")
    ...
    def load_statistics(self, update=False):
        ...
        if args.tts_backend == "tortoise":
            logs = sorted([f'{self.training_dir}/finetune/{d}' for d in os.listdir(f'{self.training_dir}/finetune/') if
                           d[-4:] == ".log"])
        else:
            log_dir = "logs"
            logs = sorted(
                [f'{self.training_dir}/{log_dir}/{d}/log.txt' for d in os.listdir(f'{self.training_dir}/{log_dir}/')])
    ...
    def cleanup_old(self, keep=2):
        ...
        models = sorted(
            [int(d[:-8]) for d in os.listdir(f'{self.training_dir}/finetune/models/') if d[-8:] == "_gpt.pth"])
        states = sorted(
            [int(d[:-6]) for d in os.listdir(f'{self.training_dir}/finetune/training_state/') if d[-6:] == ".state"])
...
def run_training(config_path, verbose=False, keep_x_past_checkpoints=0, progress=gr.Progress(track_tqdm=True)):
    ...
    for line in iter(training_state.process.stdout.readline, ""):
        ...
        result, percent, message = training_state.parse(line=line, verbose=verbose,
                                                        keep_x_past_checkpoints=keep_x_past_checkpoints,
                                                        progress=progress)
...
def whisper_transcribe(file, language=None):
    ...
    if args.whisper_backend == "m-bain/whisperx":
        ...
        if language == "ja":
            ...
            chunk_size = 30
        result = whisper_model.transcribe(file, batch_size=args.whisper_batchsize, language=language,
...
def slice_dataset(voice, trim_silence=True, start_offset=0, end_offset=0, results=None, progress=gr.Progress()):
    ...
    for filename in results:
        ...
        for segment in result['segments']:
            ...
            sliced, error = slice_waveform(waveform, sample_rate, segment['start'] + start_offset,
                                           segment['end'] + end_offset, trim_silence)
            if error:
...
def prepare_dataset(voice, use_segments=False, text_length=0, audio_length=0, progress=gr.Progress()):
    ...
    for filename in tqdm(results, desc="Parsing results"):
        ...
        if use_segment and not use_segments:
            ...
            for segment in result['segments']:
                ...
                path = f'{indir}/audio/' + filename.replace(f".{extension}",
                                                            f"_{pad(segment['id'], 4)}.{out_extension}")
        ...
        if not use_segment:
            ...
            for segment in result['segments']:
                ...
                if waveform:
                    sliced, error = slice_waveform(waveform[0], waveform[1], segment['start'] + start_offset,
                                                   segment['end'] + end_offset, trim_silence)
                    if error:
                        message = f"{error}, skipping... {file}"
                        print(message)
    ...
    messages.append(
        f"Prepared {len(lines['training'])} lines (validation: {len(lines['validation'])}, culled: {errored}).\n{training_joined}\n\n{validation_joined}")
    return "\n".join(messages)
...
def optimize_training_settings(**kwargs):
    ...
    if settings['batch_size'] / settings['gradient_accumulation_size'] < 2:
        ...
        messages.append(
            f"Gradient accumulation size is too large for a given batch size, clamping gradient accumulation size to: {settings['gradient_accumulation_size']}")
        ...
        messages.append(
            f"Batch size is not evenly divisible by the gradient accumulation size, adjusting gradient accumulation size to: {settings['gradient_accumulation_size']}")
    ...
    if settings['batch_size'] % settings['gpus'] != 0:
        ...
        if settings['batch_size'] == 0:
            settings['batch_size'] = 1
        messages.append(
            f"Batch size not neatly divisible by GPU count, adjusting batch size to: {settings['batch_size']}")
    ...
    if batch_ratio > batch_cap:
        ...
        messages.append(
            f"Batch ratio ({batch_ratio}) is expected to exceed your VRAM capacity ({'{:.3f}'.format(vram)}GB, suggested {batch_cap} batch size cap), adjusting gradient accumulation size to: {settings['gradient_accumulation_size']}")
    ...
    if settings['epochs'] < settings['save_rate']:
        ...
        messages.append(
            f"Save rate is too small for the given iteration step, clamping save rate to: {settings['save_rate']}")

    if settings['epochs'] < settings['validation_rate']:
        settings['validation_rate'] = settings['epochs']
        messages.append(
            f"Validation rate is too small for the given iteration step, clamping validation rate to: {settings['validation_rate']}")
    ...
    messages.append(
        f"For {settings['epochs']} epochs with {lines} lines in batches of {settings['batch_size']}, iterating for {iterations} steps ({steps}) steps per epoch)")
...
def save_training_settings(**kwargs):
    ...
    if not os.path.exists(settings['validation_path']):
        ...
        if validation_lines < settings['validation_batch_size']:
            ...
            messages.append(
                f"Batch size exceeds validation dataset size, clamping validation batch size to {validation_lines}")
    ...
    if settings['learning_rate_scheme'] == "MultiStepLR":
        ...
        settings['learning_rate_schedule'] = schedule_learning_rate(iterations_per_epoch,
        ...
        settings['learning_rate_restarts'] = [iterations_per_epoch * (x + 1) * restart_period for x in
                                              range(restarts)]  # [52, 104, 156, 208]

        if 'learning_rate_restart_weights' not in settings:
            settings['learning_rate_restart_weights'] = [(restarts - x - 1) / restarts for x in
                                                         range(restarts)]  # [.75, .5, .25, .125]
...
def get_autoregressive_models(dir="./models/finetunes/", prefixed=False, auto=False):
    ...
    for training in os.listdir(f'./training/'):
        if not os.path.isdir(f'./training/{training}/') or not os.path.isdir(
            f'./training/{training}/finetune/') or not os.path.isdir(f'./training/{training}/finetune/models/'):
            continue
        models = sorted(
            [int(d[:-8]) for d in os.listdir(f'./training/{training}/finetune/models/') if d[-8:] == "_gpt.pth"])
...
def get_dataset_list(dir="./training/"):
    return sorted([d for d in os.listdir(dir) if
                   os.path.isdir(os.path.join(dir, d)) and "train.txt" in os.listdir(os.path.join(dir, d))])


def get_training_list(dir="./training/"):
    if args.tts_backend == "tortoise":
        return sorted([f'./training/{d}/train.yaml' for d in os.listdir(dir) if
                       os.path.isdir(os.path.join(dir, d)) and "train.yaml" in os.listdir(os.path.join(dir, d))])
    else:
        return sorted([f'./training/{d}/config.yaml' for d in os.listdir(dir) if
                       os.path.isdir(os.path.join(dir, d)) and "config.yaml" in os.listdir(os.path.join(dir, d))])
...
def setup_args(cli=False):
    ...
    parser.add_argument("--use-deepspeed", action='store_true', default=default_arguments['use-deepspeed'],
                        help="Use deepspeed for speed bump.")
    parser.add_argument("--use-hifigan", action='store_true', default=default_arguments['use-hifigan'],
                        help="Use Hifigan instead of Diffusion")
    parser.add_argument("--use-rvc", action='store_true', default=default_arguments['use-rvc'],
                        help="Run the outputted audio thorugh RVC")
    parser.add_argument("--rvc-model", action='store_true', default=default_arguments['rvc-model'],
                        help="Specifies RVC model to use")
    ...
    parser.add_argument("--unsqueeze-sample-batches", default=default_arguments['unsqueeze-sample-batches'],
                        action='store_true', help="Unsqueezes sample batches to process one by one after sampling")
    ...
    parser.add_argument("--results-folder", type=str, default=default_arguments['results-folder'],
    ...
    parser.add_argument("--tts-backend", default=default_arguments['tts-backend'],
                        help="Specifies which TTS backend to use.")
    ...
    parser.add_argument("--diffusion-model", default=default_arguments['diffusion-model'],
                        help="Specifies which diffusion model to use for sampling.")
    parser.add_argument("--vocoder-model", default=default_arguments['vocoder-model'], action='store_true',
                        help="Specifies with vocoder to use")
    parser.add_argument("--tokenizer-json", default=default_arguments['tokenizer-json'],
                        help="Specifies which tokenizer json to use for tokenizing.")

    parser.add_argument("--phonemizer-backend", default=default_arguments['phonemizer-backend'],
                        help="Specifies which phonemizer backend to use.")

    parser.add_argument("--valle-model", default=default_arguments['valle-model'],
                        help="Specifies which VALL-E model to use for sampling.")

    parser.add_argument("--whisper-backend", default=default_arguments['whisper-backend'], action='store_true',
                        help="Picks which whisper backend to use (openai/whisper, lightmare/whispercpp)")
    ...
    parser.add_argument("--whisper-batchsize", type=int, default=default_arguments['whisper-batchsize'],
                        help="Specifies batch size for WhisperX")
    ...
    parser.add_argument("--websocket-listen-port", type=int, default=default_arguments['websocket-listen-port'],
                        help="Websocket server listen port, default: 8069")
    parser.add_argument("--websocket-listen-address", default=default_arguments['websocket-listen-address'],
                        help="Websocket server listen address, default: 127.0.0.1")
    parser.add_argument("--websocket-enabled", action='store_true', default=default_arguments['websocket-enabled'],
                        help="Websocket API server enabled, default: false")
...
def load_tts(restart=False,
    ...
    if args.tts_backend == "tortoise":
        ...
        if args.use_hifigan:
            ...
            print(
                f"Loading TorToiSe... (AR: {autoregressive_model}, diffusion: {diffusion_model}, vocoder: {vocoder_model})")
...
def load_whisper_model(language=None, model_name=None, progress=None):
    ...
    if args.whisper_backend == "openai/whisper":
        ...
        whisper_align_model = whisperx.load_align_model(
            model_name="WAV2VEC2_ASR_LARGE_LV60K_960H" if language == "en" else None, language_code=language,
            device=device)
